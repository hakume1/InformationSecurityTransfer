...
class MY_GUI():
    ...
    def set_init_window(self):
        self.init_window_name.title("信息的安全传输-客户端")  #窗口名
        self.init_window_name.geometry('800x600+350+50')                          #800 600为窗口大小，+350 +50 定义窗口弹出时的默认展示位置
        self.init_window_name["bg"] = "Coral"                                    #窗口背景色，其他背景色见：blog.csdn.net/chl0000/article/details/7657887
        self.init_window_name.attributes("-alpha",0.9)                          #虚化，值越小虚化程度越高
        #标签
        self.ip_label = Label(self.init_window_name, text="IP地址:",bg="Coral")
        self.ip_label.place(relx=0,rely=0.1)

        self.port_label = Label(self.init_window_name, text="Port端口:",bg="Coral")
        self.port_label.place(relx=0,rely=0.2)

        self.file_label = Label(self.init_window_name, text="文件名:",bg="Coral")
        self.file_label.place(relx=0,rely=0.3)

        self.log_label = Label(self.init_window_name, text="日志:",bg="Coral")
        ...
        self.file_Text = Text(self.init_window_name, width=15, height=1)  #文件
        ...
        #按钮
        self.client_button = Button(self.init_window_name, text="获取文件",bg="Azure", width=10,command=self.client)  # 调用内部方法  加()为直接调用
